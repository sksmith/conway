...
func CalculateFaceNormal(vertices []*Vertex) (Vector3, error) {
	if len(vertices) < 3 {
		return Vector3{X: 0, Y: 0, Z: 0}, fmt.Errorf("%w: %d", ErrInsufficientVertices, len(vertices))
	}
	...
	if length < lengthTolerance {
		return Vector3{X: 0, Y: 0, Z: 0}, fmt.Errorf("%w (length: %e)", ErrDegenerateFaceNormal, length)
	}
...
func calculateFaceStats(faces map[int]*Face) (float64, float64, float64) {
	...
	return minArea, maxArea, totalFaceArea / float64(len(faces))
}
...
func calculateBoundingBox(vertices map[int]*Vertex) (Vector3, Vector3) {
	if len(vertices) == 0 {
		return Vector3{X: 0, Y: 0, Z: 0}, Vector3{X: 0, Y: 0, Z: 0}
	}
