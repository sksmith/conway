...
func TestConcurrentVertexAddition(t *testing.T) {
	...
	p := conway.NewPolyhedron("ConcurrentTest")
...
func TestConcurrentEdgeAddition(t *testing.T) {
	...
	p := conway.NewPolyhedron("ConcurrentEdgeTest")

	// Create initial vertices.
	vertices := make([]*conway.Vertex, 10)

	for i := 0; i < 10; i++ {
		vertices[i] = p.AddVertex(conway.Vector3{X: float64(i), Y: 0, Z: 0})
	}
	...
	for i := 0; i < len(vertices)-1; i++ {
		edge := p.FindEdge(vertices[i].ID, vertices[i+1].ID)
...
func TestConcurrentFaceAddition(t *testing.T) {
	...
	p := conway.NewPolyhedron("ConcurrentFaceTest")

	// Create a set of vertices for faces.
	const numVertices = 12
	vertices := make([]*conway.Vertex, numVertices)

	for i := 0; i < numVertices; i++ {
		vertices[i] = p.AddVertex(conway.Vector3{
			X: float64(i % 4),
			Y: float64(i / 4),
			Z: 0,
		})
...
func TestConcurrentOperations(t *testing.T) {
	...
	p := conway.NewPolyhedron("ConcurrentMixedTest")

	// Create initial polyhedron.
	initialVertices := make([]*conway.Vertex, 4)

	for i := 0; i < 4; i++ {
		initialVertices[i] = p.AddVertex(conway.Vector3{
			X: float64(i % 2),
			Y: float64(i / 2),
			Z: 0,
		})
...
func TestConcurrentRemovalOperations(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		go func(goroutineID int) {
			...
			case 0:
				// Add new vertices.
				cube.AddVertex(conway.Vector3{X: 10, Y: float64(goroutineID), Z: 0})
			case 1:
...
func TestCentroidCachingRace(t *testing.T) {
	...
	p := conway.NewPolyhedron("CentroidRaceTest")

	// Add vertices to the polyhedron.
	for i := 0; i < 10; i++ {
		p.AddVertex(conway.Vector3{X: float64(i), Y: float64(i), Z: float64(i)})
	}
	...
	centroidChan := make(chan conway.Vector3, numGoroutines)
	...
	var firstCentroid *conway.Vector3

	for centroid := range centroidChan {
		if firstCentroid == nil {
			firstCentroid = &centroid
		} else if math.Abs(centroid.X-firstCentroid.X) > 1e-10 ||
...
func TestBoundingBoxCalculationRace(t *testing.T) {
	...
	p := conway.NewPolyhedron("BoundingBoxRaceTest")
	...
	statsChan := make(chan *conway.GeometryStats, numGoroutines/2)
...
func TestAtomicIDGeneration(t *testing.T) {
	...
	p := conway.NewPolyhedron("AtomicIDTest")
	...
	for i := 0; i < numGoroutines; i++ {
		go func() {
			...
			for j := 0; j < verticesPerGoroutine; j++ {
				v := p.AddVertex(conway.Vector3{X: float64(j), Y: 0, Z: 0})
