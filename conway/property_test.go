...
// Testconway.DualInvolution tests the dual involution property: dd(P) = P
...
func TestDualInvolution(t *testing.T) {
	...
	testCases := []struct {
		...
		{"conway.Tetrahedron", conway.Tetrahedron},
		{"conway.Cube", conway.Cube},
		{"conway.Octahedron", conway.Octahedron},
		{"conway.Dodecahedron", conway.Dodecahedron},
		{"conway.Icosahedron", conway.Icosahedron},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			original := tc.poly()
			dual1 := conway.Dual(original)
			dual2 := conway.Dual(dual1)

			// Check that dd(P) has same topology as P.
			if len(dual2.Vertices) != len(original.Vertices) {
				t.Errorf("conway.Dual involution failed: vertex count mismatch. Original: %d, dd(P): %d",
					len(original.Vertices), len(dual2.Vertices))
			}

			if len(dual2.Edges) != len(original.Edges) {
				t.Errorf("conway.Dual involution failed: edge count mismatch. Original: %d, dd(P): %d",
					len(original.Edges), len(dual2.Edges))
			}

			if len(dual2.Faces) != len(original.Faces) {
				t.Errorf("conway.Dual involution failed: face count mismatch. Original: %d, dd(P): %d",
			...
			if dual2.EulerCharacteristic() != original.EulerCharacteristic() {
				t.Errorf("conway.Dual involution failed: Euler characteristic mismatch. Original: %d, dd(P): %d",
...
func TestEulerCharacteristicPreservation(t *testing.T) {
	...
	operations := []struct {
		...
		{"conway.Dual", conway.Dual},
		{"conway.Ambo", conway.Ambo},
		{"conway.Truncate", conway.Truncate},
		{"conway.Kis", conway.Kis},
		{"conway.Join", conway.Join},
	}
...
// Testconway.DualVertexFaceCorrespondence tests the vertex-face correspondence in dual operation.
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(original.Vertices) != len(dual.Faces) {
				t.Errorf("conway.Dual operation failed vertex-face correspondence. Original vertices: %d, conway.Dual faces: %d",
					len(original.Vertices), len(dual.Faces))
			}

			if len(original.Faces) != len(dual.Vertices) {
				t.Errorf("conway.Dual operation failed face-vertex correspondence. Original faces: %d, conway.Dual vertices: %d",
					len(original.Faces), len(dual.Vertices))
			}

			// Edge count should remain the same.
			if len(original.Edges) != len(dual.Edges) {
				t.Errorf("conway.Dual operation failed edge preservation. Original edges: %d, conway.Dual edges: %d",
...
// Testconway.AmboDoubleProperty tests that ambo applied twice (expand) produces predictable results.
func TestAmboDoubleProperty(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			original := tc.poly()
			ambo1 := conway.Ambo(original)
			expand := conway.Ambo(ambo1) // aa = expand
...
func TestOperationComposition(t *testing.T) {
	...
	truncated := conway.Truncate(cube)
	dtC := conway.Dual(truncated)

	if !truncated.IsValid() {
		t.Error("conway.Truncated cube is invalid")
	}

	if !dtC.IsValid() {
		t.Error("conway.Dual of truncated cube is invalid")
	}
